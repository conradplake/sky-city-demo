...
public class SkyCity extends DoubleBufferedFrame {
	...
	public void paint(Graphics g) {
		...
		ImageRaster finalFrame = addLightCreepFilter(frame);
	...
	private ImageRaster addLightCreepFilter(ImageRaster inputFrame) {
		...
		for (int y = 0; y < inputFrame.getHeight(); y++) {
			for (int x = halfFilterLength; x < inputFrame.getWidth() - halfFilterLength; x++) {
				...
				avgRed /= filterKernel.length;
				avgGreen /= filterKernel.length;
				avgBlue /= filterKernel.length;
				
//				int newRed = Math.max(curRed, (int)avgRed);
//				int newGreen = Math.max(curGreen, (int)avgGreen);
//				int newBlue = Math.max(curBlue, (int)avgBlue);

				int newRgb = ((255 & 0xFF) << 24) | (((int) avgRed & 0xFF) << 16) | (((int) avgGreen & 0xFF) << 8)
						| (((int) avgBlue & 0xFF) << 0);
//				int newRgb = ((255 & 0xFF) << 24) | ((newRed & 0xFF) << 16) | ((newGreen & 0xFF) << 8)
//						| ((newBlue & 0xFF) << 0);
